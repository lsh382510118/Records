...
exports.insertUser = (value) => {
	let _sql =
		'insert into user set name=?, mobile=?, password=?, timezone=?, wechat=?, description=?; ';
...
exports.inserArticle = (value) => {
	let _sql =
		'insert into article set thumb=?, title=?, description=?, type=?, author=?, content=?, clicks=0, addtime=?';
	return query(_sql, value);
};
...
exports.updateArticle = (value) => {
	let _sql =
		'update article set title=?, description=?, type=?, author=?, content=?, addtime=? where id=?';
	return query(_sql, value);
};
